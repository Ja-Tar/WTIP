...
function applySettings (load = false) {
  ...
  const displayTrainsWithCargo = document.getElementById('display_train_with_cargo')
  const displayTrainWithoutTrackNr = document.getElementById('display_train_without_track_nr')
  const displayTrainThatDoesNotStop = document.getElementById('display_train_without_stop')
...
function loadFrames () {
  ...
  for (let i = 0; i < track_display.length; i++) {
    ...
    const _operator = window.operatorFullNames[operator]
...
function getProcessedData (display_id, smallestDisplayId) {
  ...
  const dataToDisplay = window.dataToDisplay
  ...
  for (let i = 0; i < dataToDisplay.length; i++) {
    ...
    if (dataToDisplay[i].track === display_id) {
      const arrivalRealTimestamp = dataToDisplay[i].arrivalRealTimestamp
      const trainNo = dataToDisplay[i].trainNo
  ...
  const trainNo = dataToDisplay[catIndex].trainNo
  const trainCategory = dataToDisplay[catIndex].category
  const stockString = dataToDisplay[catIndex].stockString
  const arrivalDelay = dataToDisplay[catIndex].arrivalDelay
  const departureDelay = dataToDisplay[catIndex].departureDelay
  const viaStations = dataToDisplay[catIndex].viaStations
  let viaStationsMain = dataToDisplay[catIndex].viaStationsMain
  const arrivalTimestamp = dataToDisplay[catIndex].arrivalTimestamp
  const departureTimestamp = dataToDisplay[catIndex].departureTimestamp
  const firstStation = dataToDisplay[catIndex].firstStation
  const lastStation = dataToDisplay[catIndex].lastStation
  const terminatesHere = dataToDisplay[catIndex].terminatesHere
  ...
  for (let j = 0; j < window.operatorConvertData.categories.length; j++) {
    const prefixData = window.operatorConvertData.categories[j]
    const trainOperator = processedData.operator
  ...
  for (let j = 0; j < window.operatorConvertData.trainNames.length; j++) {
    const trainNameData = window.operatorConvertData.trainNames[j]
    const trainOperatorBefore = processedData.operator
  ...
  for (const station of viaStationsMain) {
...
function processTimetablesData () {
  ...
  const timetableData = window.timetablesData
  const dataToDisplay = []

  for (let i = 0; i < timetableData.length; i++) {
    if (timetableData[i].region === server) {
      const timetable = timetableData[i].timetable
      const trainNo = timetableData[i].trainNo
      const stockString = timetableData[i].stockString

      if (timetable) {
        const category = timetable.category // "EIE"
        const route = timetable.route.split('|')
        const firstStation = route[0]
        const lastStation = route[1]
        const viaStations = []
        const viaStationsMain = []
        const stopList = timetable.stopList
        ...
        for (let j = 0; j < stopList.length; j++) {
          const comments = stopList[j].comments // search for [peron],[tor] in comments
          ...
          if (stopList[j].stopNameRAW.toLowerCase() === checkpoint.toLowerCase()) {
            if (stopList[j].confirmed === 0) {
              ...
              const arrivalDelay = stopList[j].arrivalDelay
              const departureDelay = stopList[j].departureDelay
              const arrivalTimestamp = stopList[j].arrivalTimestamp
              const arrivalRealTimestamp = stopList[j].arrivalRealTimestamp
              const departureTimestamp = stopList[j].departureTimestamp
...
function showDisplays (platformsConfig) { // example showDisplays("P1-1,3; P2-2,4; ")
  ...
  for (let i = 0; i < platformsConfig.length; i++) {
    const platformNumber = platformsConfig[i].split('-')[0].split('P')[1]
    const trackNumbers = platformsConfig[i].split('-')[1].split(',')
    ...
    const platformTracks = document.createElement('div')
...
function updateTextScenery () {
  ...
  if (sceneryInput.eventListeners && pointsSelect.eventListeners) {
    ...
    sceneryInput.addEventListener('input', function () {
      const station = sceneryInput.value
...
function updatePointsSelect (station) {
  const pointsSelect = document.getElementById('point')
  const scenerySupport = document.getElementById('supported_icon').children[0]
