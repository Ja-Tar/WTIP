window.timetablesAPI_URL =
  "https://stacjownik.spythere.eu/api/getActiveTrainList";
window.sceneryAPI_URL = "https://api.td2.info.pl/?method=getStationsOnline";
window.nameCorrectionsAPI_URL =
  "https://raw.githubusercontent.com/Thundo54/tablice-td2-api/master/namesCorrections.json";
window.operatorConvertAPI_URL =
  "https://raw.githubusercontent.com/Thundo54/tablice-td2-api/master/operatorConvert.json";
window.platformsAPI_URL =
  "https://raw.githubusercontent.com/Ja-Tar/WTIP/main/platforms_info.json";
...
if (window.debug === true) {
  console.warn(
    "Debug mode enabled! Change debug to false in index.js before deployment!",
  );
...
function applySettings(load = false) {
  ...
  let displayTrainsWithCargo = document.getElementById(
    "display_train_with_cargo",
  );
  let displayTrainWithoutTrackNr = document.getElementById(
    "display_train_without_track_nr",
  );
  let displayTrainThatDoesNotStop = document.getElementById(
    "display_train_without_stop",
  );
...
function loadFrames() {
  ...
  for (let i = 0; i < track_display.length; i++) {
    let {
      ...
      empty,
      terminatesHere,
    } = getProcessedData(track_display[i].id, smallestDisplayId);
...
function getProcessedData(display_id, smallestDisplayId) {
  ...
  processedData.info_bar =
    "Uwaga, na stacji trwają testy systemu informacji pasażerskiej"; //`Tor: ${display_id}`;
    ...
    for (let k = 0; k < trainNoIs.length; k++) {
      if (trainNameData.operator === trainOperatorBefore) {
        if (trainNoIs[k] === trainNo.toString()) {
          ...
          console.debug(
            `Name: ${train_name}, Operator: ${operator}, Number: ${trainNumberPrefix} ${trainNo}`,
          );
          break;
        }
      } else {
  ...
  const checkpointIndex = viaStations.findIndex(
    (station) => station.toLowerCase() === checkpoint.toLowerCase(),
  );
  ...
  processedData.time = new Date(timeTimestamp).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  }); // "HH:MM"
  ...
  for (let i = 0; i < timetableData.length; i++) {
    if (timetableData[i].region === server) {
      ...
      if (timetable) {
        ...
        if (
          window.settings.displayTrainsWithCargo === false &&
          window.trainCategory.T.includes(shortCategory)
        ) {
          continue;
        }
        ...
        for (let j = 0; j < stopList.length; j++) {
          ...
          if (
            stopList[j].stopType.includes("ph") &&
            stopList[j].confirmed === 0
          ) {
            viaStationsMain.push(stopList[j].stopNameRAW);
          }

          if (
            stopList[j].stopNameRAW.toLowerCase() === checkpoint.toLowerCase()
          ) {
            if (stopList[j].confirmed === 0) {
              if (window.settings.displayTrainThatDoesNotStop === false) {
                if (
                  !stopList[j].stopType.includes("ph") &&
                  stopList[j].terminatesHere === false &&
                  stopList[j].beginsHere === false
                ) {
                  continue;
                }
              }
              ...
              if (comments) {
                ...
                !comments &&
                window.settings.displayTrainWithoutTrackNr === false
              ) {
                continue;
              }
...
function showDisplays(platformsConfig) {
  // example showDisplays("P1-1,3; P2-2,4; ")
  ...
  for (let i = 0; i < window.platformsData.length; i++) {
    if (window.platformsData[i].sceneryName === station) {
      for (let j = 0; j < window.platformsData[i].checkpoints.length; j++) {
        let option = document.createElement("option");
        option.value =
          window.platformsData[i].checkpoints[j].name +
          window.platformsData[i].checkpoints[j].suffix;
        option.innerHTML =
          window.platformsData[i].checkpoints[j].name +
          window.platformsData[i].checkpoints[j].suffix;
...
function updatePlatformsText() {
  ...
  for (let y = 0; y < checkpointData.length; y++) {
    const dataPoint =
      checkpointData[y].name.toLowerCase() +
      checkpointData[y].suffix.toLowerCase();
